/**
 * Similarity & Fuzzy Match Utility
 * untuk fitur "command not found" suggestion
 * 
 * Taruh file ini di: src/lib/similarity.js
 */
...
function formatSuggestionMessage(inputCommand, suggestions, prefix = '.', m) {
    ...
    // Buat daftar saran command
    let saranList = ''
    suggestions.forEach((s, i) => {
        const medal = i === 0 ? '🥇' : i === 1 ? '🥈' : '🥉'
        saranList += `${medal} \`${prefix}${s.command}\`\n`
    })

    let msg2 = `Halo kak *${name}*, kamu sedang mencari \`${prefix}${inputCommand}\` kah?\n\n` +
        `Sayangnya command \`${prefix}${inputCommand}\` tidak tersedia di bot kami.\n\n` +
        `Mungkin maksudnya:\n${saranList}\n` +
        `Ketik \`${prefix}menu\` untuk daftar lengkap command.`

    return {
        message: msg2,
        interactiveButtons: suggestions.slice(0, 2).map((s) => {
            return {
                name: 'quick_reply',
