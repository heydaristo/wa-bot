 ...
 */
function createContextInfo(jid, text) {
    const contextInfo = {
        mentionedJid: []
    };
...
async function serialize(sock, msg, store = {}) {
    ...
    m.reply = async (text, options = {}) => {
        ...
        // FIX: semua replyVariant - hilangkan externalAdReply, perbaiki syntax (koma setelah spread)
        if (replyVariant === 2) {
            contextInfo = {
                ...contextInfo
            };
        ...
        if (replyVariant === 4) {
            ...
            quotedMsg = {
                ...
                message: {
                    orderMessage: {
                        orderId: '1337',
                        thumbnail: await sharp(fsc.readFileSync('./assets/images/ourin2.jpg')).resize(300, 300).toBuffer() || null,
                        itemCount: `BOT READY`,
                        status: 'INQUIRY',
                        surface: 'CATALOG',
            ...
            quotedMsg = options.quoted !== false ? msg : undefined;
        }
...
END:VCARD`;
    ...
    m.replyWithQuote = async (text, fakeJid, fakeText, options = {}) => {
        ...
        return sock.sendMessage(m.chat, {
            text,
            contextInfo: {
                ...createContextInfo(fakeJid, fakeText),
                mentionedJid: options.mentions || []
            }
        }, {
            quoted: fakeMsg
    ...
    /**
     * Reply dengan preview (tanpa card/externalAdReply)
     */
