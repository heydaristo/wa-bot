...
let autoLinkHandler = null;
try {
  autoLinkHandler = require("../plugins/main/autolink").handler;
} catch (e) { }
...
const globalRateLimiter = new RateLimiterMemory({
    points: 10,      // naikkan jadi 10
    duration: 5,     // dalam 5 detik
    blockDuration: 3 // block lebih singkat
});
...
async function messageHandler(msg, sock, options = {}) {
  ...
  try {
    ...
    if (
      ...
      // Jika voice note tapi tidak ada command yang cocok, jangan lanjut (hindari pesan suggestion)
      if (!m.isVnCommand) return;
    }
    ...
    if (!m.isCommand) {
      ...
      if (!m.isCommand) {
        ...
        if (autoLinkHandler) {
          try {
            const linkHandled = await autoLinkHandler(m, { sock, db })
    ...
    maxResults: 2,
    minSimilarity: 0.3,  // turunkan threshold
    maxDistance: 6       // perlonggar jarak
});
        ...
        if (suggestions.length > 0) {
          ...
          await sock.sendMessage(m.chat, { text: message.message });
        }
      }
    ...
    if (plugin.config.energi > 0) {
      const ownerEnergi = config.energi?.owner ?? -1;
      const premiumEnergi = config.energi?.premium ?? 100;
...
async function groupHandler(update, sock) {
  try {
    ...
    for (let participant of participants) {
      ...
      // FIX: promote notification - ganti m.reply (m tidak ada di scope ini) ke sock.sendMessage
      if (action === "promote" && groupData.notifPromote === true) {
        ...
        await sock.sendMessage(groupJid, {
          ...
          caption:
            ...
            `> 🎉 _Selamat atas jabatan barumu!_`,
  ...
  for (const update of updates) {
    ...
    try {
      if (update.update?.message) {
        ...
        // Abaikan protocolMessage (edit, delete, dll)
        if (msg.protocolMessage) continue

        // ✅ FIX: Abaikan editedMessage supaya voice note/pesan bot yang diedit
        // tidak diproses ulang sebagai pesan baru → penyebab "Audio tidak tersedia"
        if (msg.editedMessage) continue

        // ✅ FIX: Abaikan pesan dari bot sendiri (fromMe) supaya tidak loop
