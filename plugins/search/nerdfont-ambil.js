...
async function handler(m, { sock }) {
    ...
    if(!query) return m.reply(`*NERD FONT*\n\n> Masukan nama font yang ingin didownload`)
    try {
        const res = await nerdfonts()
        const data = res.find((d, i) => d?.name.toLowerCase() === query.toLowerCase())
        sock.sendMessage(m.chat, {
            document: { url: data.download_url },
            fileName: data.name,
            mimetype: 'application/zip',
            jpegThumbnail: await sharp(await axios.get(data.preview_image, { responseType: 'arraybuffer' }).then((res) => Buffer.from(res.data))).resize(50, 50).toBuffer(),
            caption: `*Done*
