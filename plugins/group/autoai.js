...
const pluginConfig = {
    ...
    usage: '.autoai on/off -mode=<character> -type=<text|voice>',
    example: '.autoai on -mode=furina -type=voice',
    isOwner: false,
    isPremium: false,
    isGroup: true,
    isPrivate: false,
...
async function handler(m) {
    ...
    // ✅ FIX: Normalisasi em dash (—) dan en dash (–) menjadi single hyphen (-)
    // Ini terjadi karena autocorrect HP mengubah "--" menjadi "—"
    const fullArgs = (m.fullArgs || '').replace(/[—–]/g, '-')
    ...
    const modeMatch = fullArgs.match(/-mode=(\w+)/i)
    ...
    if (!mode || !['on', 'off'].includes(mode)) {
        ...
        txt += `*Penggunaan:*\n`
        txt += `> .autoai on -mode=<karakter> -type=<text|voice>\n`
        txt += `> .autoai off\n\n`
        ...
        txt += `*Contoh:*\n`
        txt += `> .autoai on -mode=furina -type=text\n`
        txt += `> .autoai on -mode=kobo -type=voice`
        return m.reply(txt)
    }
...
async function generateVoiceResponse(text, sock, chatId, quotedMsg) {
    ...
    try {
        ...
        // ✅ FIX: Validasi audioUrl sebelum fetch
