...
const pluginConfig = {
    ...
    alias: ['gema'],
    category: 'convert',
    description: 'Menambahkan efek echo ke audio',
    usage: '.echo (reply audio/video)',
    example: '.echo',
    isOwner: false,
    isPremium: false,
    isGroup: false,
...
async function handler(m, { sock }) {
    ...
    const outputPath = path.join(tempDir, `echo_${Date.now()}.mp3`)
    
    try {
        const buffer = await downloadFn()
        fs.writeFileSync(inputPath, buffer)
        
        execSync(`ffmpeg -y -i "${inputPath}" -af "aecho=0.8:0.88:60:0.4" -vn "${outputPath}"`, { stdio: 'ignore' })
